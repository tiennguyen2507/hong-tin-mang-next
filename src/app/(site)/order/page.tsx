...
export default async function OrderPage({
  ...
  return (
    <div className="w-full bg-[var(--order-bg)]">
      <div className="mx-auto grid w-full max-w-7xl gap-5 px-4 py-4 sm:gap-6 sm:px-6 sm:py-6">
        {ordered === "1" ? (
          <div
        ...
        {products.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-[#e8dfd0] bg-[var(--order-card)] p-8 text-center">
            <p className="font-medium text-[var(--order-muted)]">Chưa có món nào trong thực đơn.</p>
            <p className="mt-1 text-sm text-[var(--order-muted)]">Vào trang quản trị để thêm sản phẩm.</p>
          </div>
        ) : (
