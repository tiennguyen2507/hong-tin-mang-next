...
export function LandingSteps() {
  return (
    <section id="about" className="scroll-mt-24 bg-[var(--landing-section-cream)] py-16 md:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <h2 className="font-heading text-center text-3xl font-bold text-stone-900 md:text-4xl">
          Cách đặt hàng
        </h2>
        ...
        <div className="relative mx-auto mt-14 max-w-5xl">
          <div
            className="pointer-events-none absolute left-1/2 top-0 hidden h-full w-0 -translate-x-1/2 border-l-2 border-dashed border-orange-200 md:block"
            aria-hidden
          />
            ...
            {steps.map((s, i) => {
              ...
              return (
                <li key={s.title} className="grid items-center gap-6 md:grid-cols-2 md:gap-10">
                  <div
                    className={`rounded-3xl border border-orange-100/90 bg-white p-6 shadow-sm md:p-8 ${
                  ...
                  >
                    ...
                    <h3 className="font-heading mt-2 text-xl font-bold text-stone-900 md:text-2xl">{s.title}</h3>
                    <p className="mt-3 text-stone-600 leading-relaxed">{s.body}</p>
                    {i === steps.length - 1 ? (
                      <div className={`mt-5 ${textFirst ? "" : "flex justify-end"}`}>
                        <Link
                          href="/order"
                          className="inline-flex rounded-full bg-[var(--landing-orange)] px-5 py-2.5 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[#e07d28]"
                        >
                          Đặt món ngay
                        </Link>
                      </div>
                  ...
                  <div
                    className={`flex min-h-[140px] items-center justify-center rounded-3xl bg-gradient-to-br from-orange-50 to-amber-50 text-6xl shadow-inner md:min-h-[180px] ${
